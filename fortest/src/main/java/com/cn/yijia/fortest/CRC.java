 ...
 * @date 创建时间：2015-5-18 下午10:38:41
 * @version 1.0
 * @parameter
 * @since
 * @return
...
public class CRC {
     ...
     * @param hex
     *            需要计算的字符串
     * @return CRC16校验值 (String)
     */
    public static String calcCrc16(String hex) {
        byte[] b = DataUtils.hexStringToByte(hex);
        return calcCrc16(b);
    }
     ...
     * @param data
     *             需要计算的数组
     * @return CRC16校验值 (byte[])
     */
    public static String calcCrc16(byte[] data) {
        int crc = calcCrc16(data, 0, data.length);
        String str = String.format("%04x", crc).toUpperCase();
        return str;
    }
